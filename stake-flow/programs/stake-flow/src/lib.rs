...
//   donate                    — anyone: donate any SPL token to its protocol donation vault (ATA auto-init)
//   instant_unlock            — user: exit locked position immediately, no yield, principal only
//   partial_unstake           — user: unstake a portion of locked position after lockup + collect pro-rated yield
// ============================================================
...
pub mod stake_flow {
    ...
    /// Anyone can donate any SPL token to the protocol.
    /// The donation vault is an ATA owned by `protocol_config` for the given mint.
    /// The ATA is created via the Associated Token program if it does not yet exist.
        ...
        token_interface::transfer_checked(
            CpiContext::new(
                ...
                TransferChecked {
                    ...
                    mint: ctx.accounts.donation_token_mint.to_account_info(),
                    to: ctx.accounts.donation_vault.to_account_info(),
                    authority: ctx.accounts.donor.to_account_info(),
                },
            ),
            amount,
            ctx.accounts.donation_token_mint.decimals,
        )?;

        emit!(TokenDonated {
            donor: ctx.accounts.donor.key(),
            token_mint: ctx.accounts.donation_token_mint.key(),
            amount,
        });
    ...
    /// User instantly exits a locked position at any time — no lockup check, no yield.
    /// Only the staked principal is returned. Useful for emergency exits.
    pub fn instant_unlock(ctx: Context<InstantUnlock>) -> Result<()> {
        ...
        // SECURITY: Vault must hold enough principal
        require!(
            ctx.accounts.stake_vault.amount >= amount,
            StakeFlowError::InsufficientVaultBalance
        );
        ...
        // NOTE: UserStake account is closed via `close = user` constraint — rent returned to user
        emit!(InstantUnlocked {
            user: ctx.accounts.user.key(),
            amount,
        });
    ...
    /// User unstakes a portion of their locked position after lockup expires.
    /// Collects yield pro-rated to the unstaked portion.
    /// The remaining position continues accruing rewards from the original timestamp.
    ///
    /// `unstake_amount` must be strictly less than the full staked amount;
    /// use `unstake_locked` to close the position entirely.
    pub fn partial_unstake(ctx: Context<PartialUnstake>, unstake_amount: u64) -> Result<()> {
        let config = &ctx.accounts.protocol_config;
        let user_stake = &ctx.accounts.user_stake;

        require!(!config.is_paused, StakeFlowError::ProtocolPaused);
        require!(user_stake.is_active, StakeFlowError::StakeNotActive);
        require!(unstake_amount > 0, StakeFlowError::ZeroAmount);

        // SECURITY: Partial only — use unstake_locked for full withdrawal
        require!(
            unstake_amount < user_stake.amount,
            StakeFlowError::UseFullUnstake
        );

        // SECURITY: Lockup must have expired to collect yield
        let clock = Clock::get()?;
        require!(
            clock.unix_timestamp >= user_stake.lockup_expiry,
            StakeFlowError::LockupNotExpired
        ...
        let full_amount = user_stake.amount;
        let elapsed = (clock
            .unix_timestamp
            .checked_sub(user_stake.stake_timestamp)
            .ok_or(StakeFlowError::ArithmeticOverflow)?) as u64;

        // Total rewards accrued for the full position since stake_timestamp
        // SECURITY: u128 intermediary to avoid overflow
        let total_accrued = (full_amount as u128)
            .checked_mul(config.locked_reward_rate_bps as u128)
            .ok_or(StakeFlowError::ArithmeticOverflow)?
        ...
        // Pending = total accrued minus already-claimed debt
        let pending = total_accrued
            .checked_sub(user_stake.reward_debt)
            .ok_or(StakeFlowError::ArithmeticOverflow)?;

        // Pro-rate pending rewards to the unstaked portion
        // SECURITY: multiply before divide
        let rewards_for_portion = (pending as u128)
            .checked_mul(unstake_amount as u128)
            .ok_or(StakeFlowError::ArithmeticOverflow)?
            .checked_div(full_amount as u128)
            .ok_or(StakeFlowError::ArithmeticOverflow)? as u64;

        let total_out = unstake_amount
            .checked_add(rewards_for_portion)
            .ok_or(StakeFlowError::ArithmeticOverflow)?;
        ...
        // Transfer principal + pro-rated yield to user
        let config_seeds = &[b"protocol_config".as_ref(), &[config.bump]];
        let signer_seeds = &[&config_seeds[..]];
        ...
        // Update user_stake: reduce amount and adjust reward_debt proportionally
        // new_reward_debt = reward_debt * remaining_amount / full_amount
        // This preserves correct accounting for future reward calculations.
        let remaining_amount = full_amount
            .checked_sub(unstake_amount)
            .ok_or(StakeFlowError::ArithmeticOverflow)?;

        let new_reward_debt = (user_stake.reward_debt as u128)
            .checked_mul(remaining_amount as u128)
            .ok_or(StakeFlowError::ArithmeticOverflow)?
            .checked_div(full_amount as u128)
            .ok_or(StakeFlowError::ArithmeticOverflow)? as u64;
...
/// Donate any SPL token to the protocol's donation vault for that mint.
/// The vault is an ATA owned by `protocol_config`; created automatically if absent.
#[derive(Accounts)]
pub struct Donate<'info> {
    #[account(mut)]
    pub donor: Signer<'info>,

    /// Any SPL token mint — not required to be the staking token
    pub donation_token_mint: InterfaceAccount<'info, Mint>,

    /// Donor's token account for this mint
    #[account(
        mut,
        token::mint = donation_token_mint,
        token::authority = donor,
    )]
    pub donor_token_account: InterfaceAccount<'info, TokenAccount>,

    /// Donation vault: ATA controlled by `protocol_config` PDA for this mint.
    /// Initialised via the Associated Token program when it does not yet exist.
    #[account(
        init_if_needed,
        payer = donor,
        associated_token::mint = donation_token_mint,
        associated_token::authority = protocol_config,
        associated_token::token_program = token_program,
    )]
    pub donation_vault: InterfaceAccount<'info, TokenAccount>,
...
/// Immediately exit a locked position — no lockup check, no yield.
/// Only the staked principal is returned; the UserStake PDA is closed.
#[derive(Accounts)]
...
/// Unstake a portion of a locked position after lockup expires, collecting pro-rated yield.
/// The UserStake PDA remains open with the reduced amount and adjusted reward_debt.
#[derive(Accounts)]
...
pub enum StakeFlowError {
    ...
    #[msg("Use unstake_locked to withdraw your entire position at once")]
    UseFullUnstake,
}
